...
class Courses:
    ...
    def __init__(self, course_id, course_name, difficulty, science_ness, duration, total_users, chosen = False):
        ...
        self.total_users = total_users / 100
        self.chosen = chosen

def euclid_distance(course1, course2, science_ness_weight, duration_weight):
...
def recommend_courses(courseA, course_list, k, distance_func):
    if (courseA.course_id < 0 or courseA.course_id >= len(courses)):
        return None
    sorted_courses = []
...
if __name__ == "__main__":
    ...
    recommended = recommend_courses(courseA, courses, 3, euclid_distance)
